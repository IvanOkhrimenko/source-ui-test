...
@Component({
    selector: 'src-autocomplete',
    templateUrl: './source-autocomplete.component.html',
    styleUrls: ['./source-autocomplete.component.scss'],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => SourceAutocompleteComponent),
            multi: true
        }
    ],
    ...
    //     'class': 'src-autocomplete'
    // },
})

export class SourceAutocompleteComponent implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    @Input() public debounceTime: number = 0; // delay time while typing
    ...
    public filterList() {
        ...
        if (this.query != null && this.data) {
            ...
            this.filteredList = this.customFilter !== undefined ? this.customFilter([...this.data], this.query) : this.defaultFilterFunction() as any;
            ...
            if (history) {
                ...
                if (!existingHistory.some((existingItem:any) => !this.isTypeString(existingItem)
                    ...
                    if (!this.isTypeString(item)) {
                        ...
                        const selectedIndex = copiedExistingHistory.map((el: { [x: string]: any; }) => el[this.searchKeyword]).indexOf(item[this.searchKeyword]);
     ...
     * Remove search query
     */
    public remove(e: Event | undefined) {
        e?.stopPropagation();
        this.query = '';
        ...
        if (this.historyIdentifier) {
            ...
            if (!this.historyList.some((item) => !this.isTypeString(item)
                ...
                if (!this.isTypeString(selected)) {
                    ...
                    const selectedIndex = copiedHistoryList.map((item) => item[this.searchKeyword]).indexOf(selected[this.searchKeyword] as never);
                    copiedHistoryList.splice(selectedIndex, 1);
                    copiedHistoryList.splice(0, 0, selected as never);
                    this.saveHistoryToLocalStorage([...copiedHistoryList]);
                } else {
                    // string logic
                    const copiedHistoryList = this.historyList.slice(); // copy original historyList array
                    copiedHistoryList.splice(this.historyList.indexOf(selected as never), 1);
                    copiedHistoryList.splice(0, 0, selected as never);
                    this.saveHistoryToLocalStorage([...copiedHistoryList]);
                }
            }
